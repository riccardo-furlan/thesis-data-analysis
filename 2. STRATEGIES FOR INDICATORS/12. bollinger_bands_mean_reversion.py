...
lower_band_values = data[lower_band_col].to_numpy()
...
for i in range(1, len(close_prices)):
    ...
    current_lower_band = lower_band_values[i]
    prev_lower_band = lower_band_values[i-1]
    current_middle_band = middle_band_values[i]
    prev_middle_band = middle_band_values[i-1]

    # Check signal conditions
    is_long_signal = (prev_close > prev_lower_band) and (current_close <= current_lower_band) # Long Signal (price crosses below lower band)
    is_exit_signal = (prev_close < prev_middle_band) and (current_close >= current_middle_band) # Exit Signal (price crosses above middle band)
...
filename = f"report_{ticker_symbol}_BBands_{bb_length}-{bb_std}_MeanReversion_LongOnly.csv"

try:
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        f.write("--- Backtest Summary ---\n")
        f.write(f"Indicator Used: Bollinger Bands(length:{bb_length}, std:{bb_std})\n")
        f.write("Strategy Implemented: Mean Reversion (Long Only)\n")
    ...
    print(f"Asset: {ticker_symbol} | Strategy: Bollinger Bands Mean Reversion (Long Only)")
