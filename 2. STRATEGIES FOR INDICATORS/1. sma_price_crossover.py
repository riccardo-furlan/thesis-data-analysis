...
for i in range(1, len(close_prices)):
    ...
    is_long_signal = (prev_close <= prev_sma) and (current_close > current_sma) # bullish crossover
    is_short_signal = (prev_close >= prev_sma) and (current_close < current_sma) # bearish crossover
...
filename = f"report_{ticker_symbol}_SMA{sma_period}_PriceCrossover_LongOnly.csv"

try:
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        f.write("--- Backtest Summary ---\n")
        f.write(f"Indicator Used: SMA({sma_period})\n")
    ...
    print(f"Asset: {ticker_symbol} | Strategy: Price Crossover SMA({sma_period}) (Long Only)")
