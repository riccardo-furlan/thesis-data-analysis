...
# Calculate ALMAs
data['ALMA_Fast'] = alma(data['Close'], alma_fast_window, alma_sigma, alma_offset)
data['ALMA_Slow'] = alma(data['Close'], alma_slow_window, alma_sigma, alma_offset)
data.dropna(inplace=True) # Remove rows with NaN values due to ALMA calculation

# BACKTEST EXECUTION

close_prices = data['Close'].to_numpy()
alma_fast_values = data['ALMA_Fast'].to_numpy()
alma_slow_values = data['ALMA_Slow'].to_numpy()
dates = data.index
...
for i in range(1, len(close_prices)):
    ...
    current_alma_fast = alma_fast_values[i]
    current_alma_slow = alma_slow_values[i]
    prev_alma_fast = alma_fast_values[i-1]
    prev_alma_slow = alma_slow_values[i-1]

    # Check crossover conditions
    is_long_signal = (prev_alma_fast <= prev_alma_slow) and (current_alma_fast > current_alma_slow) # bullish crossover (golden cross)
    is_short_signal = (prev_alma_fast >= prev_alma_slow) and (current_alma_fast < current_alma_slow) # bearish crossover (death cross)
...
filename = f"report_{ticker_symbol}_DualALMACrossover_{alma_fast_window}-{alma_slow_window}_LongOnly.csv"

try:
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        f.write("--- Backtest Summary ---\n")
        f.write(f"Indicator Used: ALMA(w:{alma_fast_window}) / ALMA(w:{alma_slow_window})\n")
        f.write("Strategy Implemented: Moving Average Crossover (Long Only)\n")
    ...
    print(f"Asset: {ticker_symbol} | Strategy: MA Crossover ALMA ({alma_fast_window}/{alma_slow_window}) (Long Only)")
