...
data.dropna(inplace=True) # Remove rows with NaN values due to SMA calculation

# BACKTEST EXECUTION

close_prices = data['Close'].to_numpy()
sma_fast_values = data['SMA_Fast'].to_numpy()
sma_slow_values = data['SMA_Slow'].to_numpy()
dates = data.index
...
for i in range(1, len(close_prices)):
    ...
    current_sma_fast = sma_fast_values[i]
    current_sma_slow = sma_slow_values[i]
    prev_sma_fast = sma_fast_values[i-1]
    prev_sma_slow = sma_slow_values[i-1]

    # Check crossover conditions
    is_long_signal = (prev_sma_fast <= prev_sma_slow) and (current_sma_fast > current_sma_slow) # bullish crossover (golden cross)
    is_short_signal = (prev_sma_fast >= prev_sma_slow) and (current_sma_fast < current_sma_slow) # bearish crossover (death cross)
...
filename = f"report_{ticker_symbol}_DualSMACrossover_{sma_fast_period}-{sma_slow_period}_LongOnly.csv"

try:
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        f.write("--- Backtest Summary ---\n")
        f.write(f"Indicator Used: SMA({sma_fast_period}) / SMA({sma_slow_period})\n")
    ...
    print(f"Asset: {ticker_symbol} | Strategy: MA Crossover SMA({sma_fast_period}/{sma_slow_period}) (Long Only)")
