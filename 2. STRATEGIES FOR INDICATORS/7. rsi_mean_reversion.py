...
data.dropna(inplace=True)  # Remove rows with NaN values due to RSI calculation
...
for i in range(1, len(close_prices)):
    ...
    is_long_signal = (prev_rsi <= oversold_level) and (current_rsi > oversold_level) # Long Signal (crosses 30 from below)
    is_exit_signal = (prev_rsi >= overbought_level) and (current_rsi < overbought_level) # Exit Signal (crosses 70 from above)
...
filename = f"report_{ticker_symbol}_RSI{rsi_period}_MeanReversion_LongOnly.csv"

try:
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        f.write("--- Backtest Summary ---\n")
        f.write(f"Indicator Used: RSI({rsi_period})\n")
        f.write(f"Strategy Implemented: Oversold/Overbought (Levels {oversold_level}/{overbought_level}) (Long Only)\n")
    ...
    print(f"Asset: {ticker_symbol} | Strategy: RSI OverSold/OverBought (Long Only)")
