...
data.dropna(inplace=True) # Remove rows with NaN values due to Donchian Channels calculation

# BACKTEST EXECUTION

close_prices = data['Close'].to_numpy()
upper_channel_values = data[upper_channel_col].to_numpy()
middle_channel_values = data[middle_channel_col].to_numpy()
dates = data.index
...
for i in range(1, len(close_prices)):
    ...
    current_upper_channel = upper_channel_values[i]
    prev_upper_channel = upper_channel_values[i-1]
    current_middle_channel = middle_channel_values[i]
    prev_middle_channel = middle_channel_values[i-1]

    # Check signal conditions
    is_long_signal = (prev_close < prev_upper_channel) and (current_close >= prev_upper_channel) # Long Signal
    is_exit_signal = (prev_close > prev_middle_channel) and (current_close <= prev_middle_channel) # Exit Signal
...
filename = f"report_{ticker_symbol}_Donchian_{dc_period}_Breakout_LongOnly.csv"

try:
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        f.write("--- Backtest Summary ---\n")
        f.write(f"Indicator Used: Donchian Channels(period:{dc_period})\n")
    ...
    print(f"Asset: {ticker_symbol} | Strategy: Donchian Channels Breakout (Long Only)")
