...
data.dropna(inplace=True) # Remove rows with NaN values due to Zero-Lag MACD calculation

# BACKTEST EXECUTION

close_prices = data['Close'].to_numpy()
zlmacd_line_values = data['ZLMACD'].to_numpy()
signal_line_values = data['ZLMACD_signal'].to_numpy()
dates = data.index
...
for i in range(1, len(close_prices)):
    ...
    current_zlmacd_line = zlmacd_line_values[i]
    prev_zlmacd_line = zlmacd_line_values[i-1]
    current_signal_line = signal_line_values[i]
    prev_signal_line = signal_line_values[i-1]

    # Check crossover conditions
    is_long_signal = (prev_zlmacd_line <= prev_signal_line) and (current_zlmacd_line > current_signal_line) # bullish crossover
    is_exit_signal = (prev_zlmacd_line >= prev_signal_line) and (current_zlmacd_line < current_signal_line) # bearish crossover
...
filename = f"report_{ticker_symbol}_ZLMACD_{fast_period}-{slow_period}-{signal_period}_SignalCross_LongOnly.csv"

try:
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        f.write("--- Backtest Summary ---\n")
        f.write(f"Indicator Used: Zero-Lag MACD({fast_period}, {slow_period}, {signal_period})\n")
        f.write("Strategy Implemented: ZL MACD/Signal Line Crossover (Long Only)\n")
    ...
    print(f"Asset: {ticker_symbol} | Strategy: Zero-Lag MACD Signal Line Crossover (Long Only)")
