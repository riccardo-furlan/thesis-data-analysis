...
for i in range(1, len(close_prices)):
    ...
    is_long_signal = (prev_close <= prev_ema) and (current_close > current_ema) # bullish crossover
    is_short_signal = (prev_close >= prev_ema) and (current_close < current_ema) # bearish crossover
...
filename = f"report_{ticker_symbol}_EMA{ema_period}_PriceCrossover_LongOnly.csv"

try:
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        f.write("--- Backtest Summary ---\n")
        f.write(f"Indicator Used: EMA({ema_period})\n")
    ...
    print(f"Asset: {ticker_symbol} | Strategy: Price Crossover EMA({ema_period}) (Long Only)")
