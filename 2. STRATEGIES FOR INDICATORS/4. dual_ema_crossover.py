...
data.dropna(inplace=True) # Remove rows with NaN values due to EMA calculation

# BACKTEST EXECUTION

close_prices = data['Close'].to_numpy()
ema_fast_values = data['EMA_Fast'].to_numpy()
ema_slow_values = data['EMA_Slow'].to_numpy()
dates = data.index
...
for i in range(1, len(close_prices)):
    ...
    current_ema_fast = ema_fast_values[i]
    current_ema_slow = ema_slow_values[i]
    prev_ema_fast = ema_fast_values[i-1]
    prev_ema_slow = ema_slow_values[i-1]

    # Check crossover conditions
    is_long_signal = (prev_ema_fast <= prev_ema_slow) and (current_ema_fast > current_ema_slow) # bullish crossover (golden cross)
    is_short_signal = (prev_ema_fast >= prev_ema_slow) and (current_ema_fast < current_ema_slow) # bearish crossover (death cross)
...
filename = f"report_{ticker_symbol}_DualEMACrossover_{ema_fast_period}-{ema_slow_period}_LongOnly.csv"

try:
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        f.write("--- Backtest Summary ---\n")
        f.write(f"Indicator Used: EMA({ema_fast_period}) / EMA({ema_slow_period})\n")
    ...
    print(f"Asset: {ticker_symbol} | Strategy: MA Crossover EMA ({ema_fast_period}/{ema_slow_period}) (Long Only)")
