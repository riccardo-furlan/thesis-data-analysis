...
data.dropna(inplace=True) # Remove rows with NaN values due to MACD calculation

# BACKTEST EXECUTION

close_prices = data['Close'].to_numpy()
macd_line_values = data[macd_line_col].to_numpy()
signal_line_values = data[macd_signal_col].to_numpy()
dates = data.index
...
filename = f"report_{ticker_symbol}_MACD_{fast_period}-{slow_period}-{signal_period}_SignalCross_LongOnly.csv"

try:
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        f.write("--- Backtest Summary ---\n")
        f.write(f"Indicator Used: MACD({fast_period}, {slow_period}, {signal_period})\n")
        f.write("Strategy Implemented: MACD/Signal Line Crossover (Long Only)\n")
    ...
    print(f"Asset: {ticker_symbol} | Strategy: MACD Signal Line Crossover (Long Only)")
